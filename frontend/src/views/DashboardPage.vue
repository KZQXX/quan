  ...
  <div class="min-h-[100dvh] bg-surface-50">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <section class="grid gap-6 lg:grid-cols-2">
        ...
        <div class="glass-card p-6"><h2 class="text-xl font-bold text-surface-900">快速打卡</h2><div class="mt-4 flex gap-2"><button v-for="tab in [['feeding','进食'], ['excretion','排便'], ['behavior','行为']] as const" :key="tab[0]" class="rounded-lg px-3 py-2 text-sm" :class="activeTab === tab[0] ? 'bg-primary-600 text-white' : 'bg-surface-100 text-surface-500'" @click="activeTab = tab[0]">{{ tab[1] }}</button></div><form v-if="activeTab === 'feeding'" class="mt-4 flex flex-col gap-3 sm:flex-row" @submit.prevent="quickFeed"><input v-model="foodType" required placeholder="食物类型" class="flex-1 rounded-xl border-surface-200"/><input v-model.number="amount" type="number" min="0" placeholder="克数（可选）" class="w-36 rounded-xl border-surface-200"/><button :disabled="!selectedPetId" class="btn-primary">记录</button></form><button v-else :disabled="!selectedPetId" class="btn-primary mt-4" @click="quickRecord">一键记录{{ activeTab === 'excretion' ? '正常排便' : '日常行为' }}</button><p v-if="message" class="mt-3 text-sm text-green-700">{{ message }}</p></div>
      </section>
    </main>
  </div>
</template>
